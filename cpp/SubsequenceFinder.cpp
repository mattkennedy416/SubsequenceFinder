...
SubsequenceFinder::SubsequenceFinder()
{
	printf("C++ class initialized\n");
}
...
int comp(const void *a, const void* b) // *** NOTE that this needs to be a static function instead of class function ***
{   Index* x = (Index*)a;
    Index* y = (Index*)b;
...
void SubsequenceFinder::init(deque *d, int capacity)
{
    d->capacity = capacity;
    d->size = 0;
...
/// Destroy the queue
void SubsequenceFinder::destroy(deque *d)
{
    free(d->dq);
}

/// Insert to the queue at the back
void SubsequenceFinder::push_back(struct deque *d, int v)
{
    d->dq[d->r] = v;
    d->r--;
    if (d->r < 0)
...
void SubsequenceFinder::pop_front(struct deque *d)
{
    d->f--;
    if (d->f < 0)
        d->f = d->capacity-1;
...
void SubsequenceFinder::pop_back(struct deque *d)
{
...
int SubsequenceFinder::front(struct deque *d)
{
    int aux = d->f - 1;
...
int SubsequenceFinder::back(struct deque *d)
{
...
int SubsequenceFinder::empty(struct deque *d)
{
    return d->size == 0;
}
...
void SubsequenceFinder::lower_upper_lemire(double *t, int len, int r, double *l, double *u)
{
    struct deque du, dl;
...
void SubsequenceFinder::printArray(double *x, int len)
{   for(int i=0; i<len; i++)
        printf(" %6.2lf",x[i]);
    printf("\n");
}
...
void SubsequenceFinder::error(int id)
{
    if(id==1)
...
void SubsequenceFinder::search(float* query, int queryLength, float* data, int dataLength, float errorLimit, float sizeOfWarpingWindows)
{
	...
	int m=-1, r=-1;
	//long long loc = 0; // ORIGINAL -- RATHER THAN STORING BEST, WE WANT TO STORE ALL LOCATIONS
    ...
    /// Read query file
    //bsf = 15.0; // TEMPORARY PLACEHOLD -- NOTE THAT THIS SHOULD BE AN INPUT PARAMETER -- ALSO NOTE THAT THIS IS THE MAX DISTANCE ***SQUARED***
    bsf = errorLimit*errorLimit;
    i = 0;
    j = 0;
    ex = ex2 = 0;

    for (int i=0; i<m; i++) // replacement for query IO
    {
    	ex += query[i];
    	ex2 += query[i]*query[i];
    	q[i] = query[i];
	...
	// if we ever have a case where length of data goes beyond a single epoch, the indices here will reset to zero...
	// so pretty sure we need to independently keep track of the total index in data like fscanf would
	// but, also pretty sure it would just be (it*EPOCH + relativeInd)?
	// where *it* is the the EPOCH iteration
...
//                    buffer[k] = d;
        		buffer[k] = data[it*EPOCH + k];
        	}

        }
        else
        {
        	printf("Starting epoch %i, copying end data from prev epoch to start of this one ... ", it);
        	for(k=0; k<m-1; k++)
                buffer[k] = buffer[EPOCH-m+1+k]; // so pretty sure this intializes additional epochs with data from end of previous epoch
        	printf("success!\n");
        }
...
//                break;
//            buffer[ep] = d;
        	if (it*EPOCH + ep >= dataLength)
        		break;
        	buffer[ep] = data[it*EPOCH + ep];
            ep++;
        }
						...
						if (lb_k < bsf)
						{
							...
							if (lb_k2 < bsf)
							{
								...
								{   /// Update bsf
									...
									//bsf = dist; // DON'T UPDATE -- WE'RE TAKING EVERYTHING BELOW THIS, NOT JUST THE BEST
									//loc = (it)*(EPOCH-m+1) + i-m+1; // ORIGINAL
									loc.push_back((it)*(EPOCH-m+1) + i-m+1); // NEW -- ADD VALUE TO OUR NEW VECTOR
									locDist.push_back(sqrt(dist));
								}
							} else
								keogh2++;
						} else
	...
	//cout << "Location : " << loc << endl; // ORIGINAL, WHICH IS UNABLE TO PRINT OUR NEW VECTOR
	for (int n=0; n<loc.size(); n++) // NEW
		std::cout << "Location : " << loc[n] << " -- Distance : " << locDist[n] << std::endl; // NEW

	//cout << "Distance : " << sqrt(bsf) << endl;
	std::cout << "Data Scanned : " << i << std::endl;
	std::cout << "Total Execution Time : " << (t2-t1)/CLOCKS_PER_SEC << " sec" << std::endl;
...
int SubsequenceFinder::getResultsLength()
{
	return loc.size();
}


void SubsequenceFinder::getResults(int* locations, float* distances)
{
	for (int n=0; n<loc.size(); n++)
	{
		locations[n] = loc[n];
		distances[n] = locDist[n];
	}
}
...
//
//SubsequenceFinder::SubsequenceFinder(int X0, int Y0, int X1, int Y1)
//{
//	x0 = X0;
//	y0 = Y0;
//	x1 = X1;
//	y1 = Y1;
//
//	printf("Setting bounds x=(%i,%i), y=(%i,%i)\n", x0, x1, y0, y1);
//}
//
//SubsequenceFinder::~SubsequenceFinder()
//{
...
//}
//
//int SubsequenceFinder::getHeight()
//{
//	return (y1 - y0);
//}
//
//int SubsequenceFinder::getArea()
//{
//	printf("Retreiving bounds x=(%i,%i), y=(%i,%i) for getArea()\n", x0, x1, y0, y1);
//
...
//}
//
//void SubsequenceFinder::move(int dx, int dy)
//{
//	x0 += dx;
//	y0 += dy;
//	x1 += dx;
